...
function GlobeSurfaceTileProvider(options) {
  ...
  // Globe filter properties (custom extension for tech/ink style)
  this.filterEnabled = false;
  this.filterColor = new Cartesian3(1.0, 1.0, 1.0);
  this.filterExposure = 1.0;
...
function createTileUniformMap(frameState, globeSurfaceTileProvider) {
  const uniformMap = {
    ...
    // Globe filter uniforms (custom extension)
    u_globeFilterEnabled: function () {
      return this.properties.globeFilterEnabled;
    },
    u_globeFilterColor: function () {
      return this.properties.globeFilterColor;
    },
    u_globeFilterExposure: function () {
      return this.properties.globeFilterExposure;
    },
    u_globeFilterContrast: function () {
      return this.properties.globeFilterContrast;
    },
    ...
    properties: {
      ...
      // Globe filter properties (custom extension)
      globeFilterEnabled: false,
      globeFilterColor: new Cartesian3(1.0, 1.0, 1.0),
      globeFilterExposure: 1.0,
      globeFilterContrast: 1.0,
    },
  };
...
function addDrawCommandsForTile(tileProvider, tile, frameState) {
  ...
  do {
    ...
    // Sync globe filter properties
    uniformMapProperties.globeFilterEnabled = tileProvider.filterEnabled;
    if (defined(tileProvider.filterColor)) {
      Cartesian3.clone(tileProvider.filterColor, uniformMapProperties.globeFilterColor);
    }
    uniformMapProperties.globeFilterExposure = tileProvider.filterExposure;
    uniformMapProperties.globeFilterContrast = tileProvider.filterContrast;
